...
<AccordionGroup>
  ...
  <Accordion title="Transaction Stuck in Pending">
    ...
    3. If the transaction is still stuck after a manual recovery attempt, contact [support@strails.co](mailto:support@strails.co) with the `requestId` and timestamp.
...
<Info>
  If you cannot resolve an issue using this guide, email the Strails support team at [support@strails.co](mailto:support@strails.co) with your `requestId`, the full request payload (redact your API key), and the timestamp of the failed call.
</Info>
