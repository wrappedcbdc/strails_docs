...
<AccordionGroup>
  ...
  <Accordion title="User Onramp Statuses">
    ...
    **Lifecycle flow:**
    ...
    with autoSwap enabled, `funded` continues instead as:
...
<AccordionGroup>
  ...
  <Accordion title="429 Too Many Requests">
    ...
    - See [Rate Limits](/authentication#rate-limits) for per-endpoint limits. A `429` response carries a `data.retry_after` value in seconds - wait at least that long before retrying.
...
| **Kobo** | All Fee Management endpoints: `/feewithdrawal`, `/getwithdrawalhistory`, `/getaccumulatedfees`, `/fees/strails/preview`, `capFee` on `/managefees` | `50000` = ₦500.00 |
| **Human-readable token units** | `/withdrawasset`, `/swap`, `/swaptrigger`, all FX price/amount fields | `100.50` = 100.50 USDC |
| **Smallest unit (wei)** | `/fintechtransfer`, `/initiateofframp`, balances, blockchain status responses | `500000000` = 500 cNGN (6 decimals; DAI uses 18) |

<Warning>
  Fee withdrawals are **kobo**, not Naira - `"amount": 50000` on `/feewithdrawal` withdraws ₦500.00. See [Fee Management](/api-reference/fee-management-api) for the full set.
</Warning>

<Note>
  Full per-endpoint breakdown: [Amount Formatting](/payload-formats#amount-formatting).
</Note>
