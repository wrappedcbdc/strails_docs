...
| `/setuserdefaultwallet` | 20 rpm |
| `/getuserdefaultwallet` | 100 rpm |
| `/removeuserdefaultwallet` | 20 rpm |
...
| `/withdrawals` | 100 rpm | List withdrawal transfer history |
...
| `/swaphistory` | 100 rpm | List historical swaps |

### FX Orderbook & Trading

| Endpoint | Method | Rate limit | Notes |
|---|---|---|---|
| `/fx/limit-order` | POST | 30 rpm | Create orderbook order |
| `/fx/limit-order/update` | PUT | 30 rpm | Update orderbook order |
| `/fx/limit-order/delete` | DELETE | 30 rpm | Delete orderbook order |
| `/fx/limit-orders` | GET | 100 rpm | List your orders |
...
| `/fx/quote` | POST | 60 rpm | Generate trade quote |
| `/fx/trade` | POST | 30 rpm | Execute trade |
| `/fx/market-order` | POST | 30 rpm | Execute market order |
| `/fx/trades/status` | GET | 60 rpm | Get trade status |
...
<Warning>
  `response_code` is always one of the two-digit envelope codes (`00`-`05`) documented in [Payload Formats](/payload-formats#response-code-table) - it is never the HTTP status code. Branch your error handling on `response_code` plus the HTTP status, and treat the `status` string as display text only: its casing varies between endpoint families (`Success`/`success`, `Error`/`error`/`Failed`).
</Warning>
...
| `AUTHORIZATION_FAILED` | 403 | Caller IP is not on the allowlist, or the action is not permitted |
...
3. **Rotate your key regularly.** Use the [`/regenerateapikey`](/api-reference/management-api#get-regenerateapikey) endpoint to issue a new key and revoke the old one on a regular schedule.
