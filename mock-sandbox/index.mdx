...
The sandbox is exposed through a dedicated Firebase Hosting site and a single Cloud Function (`mockapigateway`).

| Environment | Base URL |
|-------------|----------|
| Test        | `https://sandbox.stablesrail.io` |
| Production  | `https://sandbox.strails.io` |
...
- **Firestore-backed keys**: Sandbox API keys are stored in `sandboxApiKeys` collection, keyed by `fintechId`.
- **No production secrets**: The sandbox uses its own parameter defaults (`SANDBOX_*`) and does not bind production `defineSecret` values.
